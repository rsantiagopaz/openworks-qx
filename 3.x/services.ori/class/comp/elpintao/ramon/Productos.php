...
*/  
  ...
  public function method_aplicar_porcentaje_recursivo($params, $error) {
	...
	if ($p->aplicar == "pcfcd") {
		$sql = "SELECT producto.iva, producto.desc_producto, producto_item.*, fabrica.desc_fabrica FROM (producto INNER JOIN producto_item USING(id_producto)) INNER JOIN fabrica USING(id_fabrica) WHERE producto.id_arbol=" . $p->id_arbol . (is_null($p->id_fabrica) ? "" : " AND producto.id_fabrica=" . $p->id_fabrica);
		$rs = mysql_query($sql);
		while ($row = mysql_fetch_object($rs)) {
		...
		$sql = "UPDATE producto INNER JOIN producto_item USING(id_producto) SET producto_item." . $p->aplicar . " = producto_item." . $p->aplicar . (($p->porcentaje > 0) ? " + " : " - ") . "(producto_item." . $p->aplicar . " * " . abs($p->porcentaje) . " / 100) WHERE producto.id_arbol=" . $p->id_arbol . (is_null($p->id_fabrica) ? "" : " AND producto.id_fabrica=" . $p->id_fabrica);
		mysql_query($sql);
		$this->transmitir($sql);
	}
