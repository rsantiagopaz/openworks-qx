qx.Class.define("elpintao.comp.remitos.windowRemito",
{
	...
	if (id_remito=="0") {
		...
		if (emitir) {
			...
			try {
				var resultado = rpc.callSync("leer_remitos_emi_detalle", {id_remito_emi: rowRemito.id_remito_emi});
			} catch (ex) {
			...
			var rpc = new qx.io.remote.Rpc("services/", "comp.Remitos");
			try {
				var resultado = rpc.callSync("leer_remitos_rec_detalle", {id_remito_rec: rowRemito.id_remito_rec});
			} catch (ex) {
				alert("Sync exception: " + ex);
			}
		}
