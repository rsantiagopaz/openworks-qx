...
*/
	...
	tblDatos.addListener("dataEdited", function(e){
		...
		if (data.value != data.oldValue) {
			...
			var rpc = new qx.io.remote.Rpc("http://pintao:1qaz2wsx@" + application.arraySucursales[p.id_sucursal].url + "/services/", "comp.elpintao.ramon.Stock");
	...
	slbSucursal.addListener("changeSelection", function(e){
		...
		for (var x in application.arraySucursales) {
			...
			var rpc = new qx.io.remote.Rpc("http://pintao:1qaz2wsx@" + application.arraySucursales[x].url + "/services/", "comp.elpintao.ramon.Stock");
			//var rpc = new qx.io.remote.Rpc("http://root@" + application.arraySucursales[x].url + "/services/", "comp.elpintao.ramon.Stock");
			rpc.setCrossDomain(true);
			rpc.callAsync(function(resultado, error, id) {
