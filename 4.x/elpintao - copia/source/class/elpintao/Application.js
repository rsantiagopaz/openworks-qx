  ...
  members :
  {
	...
	var functionActualizarPedidoExt = this.functionActualizarPedidoExt = function(parametro) {
		if (pagePedidosExt != null) {
			...
			pagePedidosExt.functionActualizar();
		}
	}
	...
	*/
	...
	var commandF5 = new qx.ui.core.Command("F5");
	var commandCtrlR = new qx.ui.core.Command("Ctrl+R");
	...
	*/
	componente.comp.elpintao.ramon.Rutinas.crear_obj_base(["objColor"]);
	...
	*/
	componente.comp.elpintao.ramon.Rutinas.crear_obj_base(["objMoneda"]);
	...
	*/
	...
	objTransporte.store.setUrl("../../services/class/comp/elpintao/ramon/Stores.php?rutina=leer_transporte");
	...
	objUsuario.store.setUrl("../../services/class/comp/elpintao/ramon/Stores.php?rutina=leer_usuario");
	
	
	
	
	var timerTransmision = this.timerTransmision = new qx.event.Timer(60000 * 1);
	...
	btnAplicarAjuste.addListener("execute", function(e){
		functionLogin(qx.lang.Function.bind(function(e) {
			var win = new componente.comp.elpintao.ramon.productos.windowAplicarAjustes("1");
	...
	btnReparar.addListener("execute", function(e){
		dialog.Dialog.confirm("Desea reparar cadenas de búsqueda? El proceso puede llevar algún tiempo, se notificará cuando finalice.", function(e){
			if (e) {
				...
				rpc.setTimeout(60000 * 15);
				rpc.callAsync(function(resultado, error, id) {
					//doc.remove(imageLoading);
	...
	var btnPedidosExt = new qx.ui.menu.Button("Pedidos externos");
	...
	var btnFabricas = new qx.ui.menu.Button("Fábricas");
	btnFabricas.addListener("execute", function(e){
		functionChequearPassword(this.rowParamet.password_general, function(e) {
