qx.Class.define("pediatra.comp.pacientes.pagePaciente",
{
	...
	var commandNuevaVisita = new qx.ui.core.Command("Insert");
	commandNuevaVisita.addListener("execute", function(e){
		var win = new pediatra.comp.pacientes.windowVisita();
	...
	var commandModificarVisita = new qx.ui.core.Command("Enter");
	...
	tableModelVisita.setColumns(["#", "Motivo", "Antecedentes", "Fecha", "Per.enc.", "Talla", "Peso", "Presión"]);
	...
	//tblVisita.setShowCellFocusIndicator(true);
	...
	//Tabla
	var tableModelVisitaParmed = new qx.ui.table.model.Simple();
	tableModelVisitaParmed.setColumns(["Descripción", "Tipo", "Código"], ["descrip", "tipo", "codigo"]);

	var custom = {tableColumnModel : function(obj) {
		return new qx.ui.table.columnmodel.Resize(obj);
	}};
	
	var tblVisitaParmed = new componente.comp.ui.ramon.table.Table(tableModelVisitaParmed, custom);
	tblVisitaParmed.toggleShowCellFocusIndicator();
	tblVisitaParmed.toggleColumnVisibilityButtonVisible();
	tblVisitaParmed.toggleStatusBarVisible();
	
	var tableColumnModel = tblVisitaParmed.getTableColumnModel();
	var resizeBehavior = tableColumnModel.getBehavior();
	//resizeBehavior.set(0, {width:"80%", minWidth: 100});
	//resizeBehavior.set(1, {width:"20%", minWidth: 100});
	
	var selectionModel = tblVisitaParmed.getSelectionModel();
	selectionModel.setSelectionMode(qx.ui.table.selection.Model.SINGLE_SELECTION);
	selectionModel.addListener("changeSelection", function(){
		var bool = (selectionModel.getSelectedCount() > 0);
	});

	composite.add(tblVisitaParmed, {left: 0, top: "83%", right: 0, bottom: 0});
	...
	tableModelVisitaParmed.setDataAsMapArray([{descrip: "Zitromax", tipo: "Medicamento", codigo: ""}, {descrip: "Torax frente de pie", tipo: "Radiografía", codigo: ""}], true)
