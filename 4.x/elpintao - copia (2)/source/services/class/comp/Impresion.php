...
case 'imprimir_pedext': {
	...
	$sql="SELECT pedido_ext.*, transporte.descrip AS transporte, fabrica.descrip AS fabrica FROM (pedido_ext INNER JOIN fabrica USING(id_fabrica)) INNER JOIN transporte USING(id_transporte) WHERE pedido_ext.id_pedido_ext='" . $_REQUEST['id_pedido_ext'] . "'";
	$rsR = mysql_query($sql);
	$rowR = mysql_fetch_object($rsR);

	$sql="SELECT pedido_ext_detalle.*, CONCAT(producto_item.cod_interno, ' - ', producto.descrip) AS producto, producto_item.capacidad, color.descrip AS color, unidad.descrip AS unidad FROM ((((pedido_ext_detalle INNER JOIN producto_item USING(id_producto_item)) INNER JOIN producto USING(id_producto)) INNER JOIN fabrica USING(id_fabrica)) INNER JOIN color USING (id_color)) INNER JOIN unidad USING (id_unidad) WHERE id_pedido_ext='" . $_REQUEST['id_pedido_ext'] . "'";
	$rsD = mysql_query($sql);
...
<tr><td>&nbsp;</td></tr>
<tr><th>Producto</th><th>Capacidad</th><th>U.</th><th>Color</th><th>Cantidad</th></tr>
...
?>
		<tr><td><?php echo $rowD->producto ?></td><td align="right"><?php echo $rowD->capacidad ?></td><td><?php echo $rowD->unidad ?></td><td><?php echo $rowD->color ?></td><td align="right"><?php echo $rowD->cantidad ?></td></tr>
		<tr><td colspan="10"><hr></td></tr>
