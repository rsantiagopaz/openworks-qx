qx.Class.define("gbio.comp.empleados.pageAsignaciones",
{
	...
	selectionModelEmpleados.addListener("changeSelection", function(e){
		...
		if (rowCount == 0 || selectionModelEmpleados.isSelectionEmpty()) {
			...
			for (var y in data1) {
				aux = new qx.ui.form.ListItem(data1[y].turno + " (" + data1[y].desde + ((data1[y].hasta == null) ? "" : " - " + data1[y].hasta) + ")" , null, data1[y].id_empleado_turno);
				slbTurno2.add(aux);
			}
		}
	});
	...
	btnEliminarTurno.addListener("execute", function(e){
		dialog.Dialog.confirm("Dese eliminar este item? Se eliminarán también los permisos relacionados.", function(e){
			if (e) {
