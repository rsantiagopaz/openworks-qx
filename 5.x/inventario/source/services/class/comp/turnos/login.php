...
//		require_once("conexsalud1.php");
	...
	function method_Login($params, $error)
    {
		...
		*/
		...
		$this->_db->setQuery("
		SELECT sistemas_perfiles_usuarios_oas.id_sist_perfil_usuario_oas as value, 
		sistemas_perfiles_usuarios_oas.id_oas_usuario, _organismos_areas.organismo_area_id,
		_organismos_areas.organismo_area AS organismo_area_descrip, _servicios.denominacion AS servicio_descrip,
		...
		INNER JOIN _organismos_areas ON _organismos_areas_servicios.id_organismo_area = _organismos_areas.organismo_area_id
		INNER JOIN _servicios ON _organismos_areas_servicios.id_servicio = _servicios.id_servicio
		ORDER BY organismo_area_descrip, servicio_descrip
		");
        $this->_db->alter();
        ...
        foreach ($result->servicios as $item) {
			if (is_null($servicios[$item->organismo_area_id])) {
				$item->perfiles = array();
				$item->perfiles[$item->perfil_id] = true;
				$servicios[$item->organismo_area_id] = $item;
			} else {
				$servicios[$item->organismo_area_id]->perfiles[$item->perfil_id] = true;
			}
        }
        $result->servicios = array();
        foreach ($servicios as $item) {
        	$item->label = $item->organismo_area_descrip . " - " . $item->servicio_descrip;
			$this->_db->setQuery("SELECT id_organismo_area_servicio FROM _organismos_areas_servicios WHERE id_organismo_area='" . $item->organismo_area_id . "' AND id_servicio='201'");
			$this->_db->alter();
			$aux = $this->_db->loadResult();
			
			$item->id_organismo_area_servicio_destino = (count($aux) == 0) ? null : $aux[0]->id_organismo_area_servicio;
