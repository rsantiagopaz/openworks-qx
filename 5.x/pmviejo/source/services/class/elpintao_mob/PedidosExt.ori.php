...
class class_PedidosExt extends class_Base
{
  ...
  public function method_leer_internos($params, $error) {
	...
	if ($p->id_fabrica == "1") {
		...
		$sql.= $WHERE . " AND producto.id_fabrica='" . $p->id_fabrica . "'";
	}
	...
	while ($rowPedidoSuc = mysql_fetch_object($rsPedidoSuc)) {
		...
		$rowPedidoSuc->remarc_final = (float) $rowPedidoSuc->remarc_final;
		$rowPedidoSuc->remarc_mayorista = (float) $rowPedidoSuc->remarc_mayorista;
		$rowPedidoSuc->desc_final = (float) $rowPedidoSuc->desc_final;
		$rowPedidoSuc->desc_mayorista = (float) $rowPedidoSuc->desc_mayorista;
		$rowPedidoSuc->bonif_final = (float) $rowPedidoSuc->bonif_final;
		$rowPedidoSuc->bonif_mayorista = (float) $rowPedidoSuc->bonif_mayorista;
		$rowPedidoSuc->comision_vendedor = (float) $rowPedidoSuc->comision_vendedor;
		
		$rowPedidoSuc->mobile = new stdClass;
		$rowPedidoSuc->mobile->title = $rowPedidoSuc->producto . ", " . $rowPedidoSuc->color . ", " . $rowPedidoSuc->capacidad . " " . $rowPedidoSuc->unidad;
		$rowPedidoSuc->mobile->subtitle = "";
		...
		$rowPedidoSuc->detallePedInt = $this->toJson("SELECT id_pedido_suc_detalle, descrip, SUM(cantidad) AS cantidad, sucursal.id_sucursal FROM (pedido_suc_detalle INNER JOIN pedido_suc USING(id_pedido_suc)) INNER JOIN sucursal USING(id_sucursal) WHERE id_producto_item = '" . $rowPedidoSuc->id_producto_item . "' AND id_pedido_ext=0 GROUP BY id_sucursal ORDER BY descrip");
		...
		$sql="SELECT id_sucursal, descrip, stock FROM stock INNER JOIN sucursal USING(id_sucursal) WHERE id_producto_item = '" . $rowPedidoSuc->id_producto_item . "' ORDER BY descrip";
