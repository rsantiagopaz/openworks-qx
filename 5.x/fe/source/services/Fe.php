...
class class_Fe
{
  ...
  public function method_fe_documento($params, $error) {
  	...
  	$factura = json_decode(json_encode($p), true);
  	$factura = $p['factura'];
	...
	$sql.= ", fecha_cbte=" . $factura['fecha_cbte'];
	$sql.= ", cae='" . $factura['cae'] . "'";
	...
	$sql.= ", entrada='" . json_encode($factura) . "'";
	$sql.= ", salida='" . json_encode($salida) . "'";
