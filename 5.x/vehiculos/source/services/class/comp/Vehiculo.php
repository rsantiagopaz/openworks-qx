...
class class_Vehiculo extends class_Base
{
  ...
  public function method_entrada_vehiculo($params, $error) {
  	...
  	$p->id_entsal = $insert_id;
  	$this->agregar_entsal_tipo_reparacion($params, $error);
  ...
  public function agregar_entsal_tipo_reparacion($params, $error) {
  	$p = $params[0];
  	
	foreach ($p->tipo_reparacion as $item) {
		$sql = "SELECT * FROM entsal_tipo_reparacion WHERE id_entsal=" . $p->id_entsal . " AND id_tipo_reparacion=" . $item;
		$rs = mysql_query($sql);
		if (mysql_num_rows($rs) == 0) {
  			$sql = "INSERT entsal_tipo_reparacion SET id_entsal=" . $p->id_entsal . ", id_tipo_reparacion=" . $item;
  			mysql_query($sql);
		}
	}
  }
  ...
  public function method_leer_entsal($params, $error) {
	...
	function functionAux1(&$row, $key) {
		$row->total = (float) $row->total;
		$row->reparaciones = null;
		$row->tipo_reparacion = array();
		
		$sql = "SELECT tipo_reparacion.* FROM entsal_tipo_reparacion INNER JOIN tipo_reparacion USING(id_tipo_reparacion) WHERE id_entsal=" . $row->id_entsal;
		$rsTipo_reparacion = mysql_query($sql);
		while ($rowTipo_reparacion = mysql_fetch_object($rsTipo_reparacion)) {
			$row->tipo_reparacion[] = $rowTipo_reparacion;
		}
	};
  ...
  public function method_leer_gral($params, $error) {
	...
	$sql = "SELECT id_entsal, organismo_area_id, CONCAT(nro_patente, '  ', marca) AS vehiculo, f_ent, f_sal, asunto, entsal.estado FROM entsal INNER JOIN vehiculo USING(id_vehiculo) WHERE vehiculo.id_parque=" . $_SESSION['parque']->id_parque . " AND (entsal.estado <> 'S' OR entsal.asunto) ORDER BY f_ent DESC";
	$rs = mysql_query($sql);
