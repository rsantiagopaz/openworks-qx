...
class class_Vehiculo extends class_Base
{
  ...
  public function method_salida_taller($params, $error) {
  	...
  	$sql = "UPDATE movimiento SET f_sal=NOW(), id_usuario_sal=1, estado='S' WHERE id_movimiento=" . $p->id_movimiento;
  	mysql_query($sql);
  ...
  public function method_entrada_taller($params, $error) {
	...
	$sql = "INSERT movimiento SET id_entsal=" . $p->id_entsal . ", cod_razon_social=" . $p->cod_razon_social . ", observa='" . $p->observa . "', f_ent=NOW(), id_usuario_ent=1, estado='E'";
	mysql_query($sql);
  ...
  public function method_leer_movimiento($params, $error) {
	...
	function functionAux(&$row, $key) {
		...
		$sql = "SELECT nick FROM usuario WHERE id_usuario=" . $row->id_usuario_ent;
		$rsUsuario = mysql_query($sql);
		$rowUsuario = mysql_fetch_object($rsUsuario);
		$row->usuario_ent = $rowUsuario->nick;
		
		if (is_null($row->id_usuario_sal)) {
			$row->usuario_sal = "";
		} else {
			$sql = "SELECT nick FROM usuario WHERE id_usuario=" . $row->id_usuario_sal;
			$rsUsuario = mysql_query($sql);
			$rowUsuario = mysql_fetch_object($rsUsuario);
			$row->usuario_sal = $rowUsuario->nick;
		}
	};
  ...
  public function method_entrada_vehiculo($params, $error) {
  	...
  	$sql = "INSERT entsal SET id_vehiculo=" . $p->id_vehiculo . ", observa='" . $p->observa . "', f_ent=NOW(), id_usuario_ent=1, resp_ent='" . $p->resp_ent . "', asunto=FALSE, estado='E'";
  	mysql_query($sql);
  	$insert_id = mysql_insert_id();
  ...
  public function method_salida_vehiculo($params, $error) {
  	...
  	$sql = "UPDATE entsal SET f_sal=NOW(), id_usuario_sal=1, resp_sal='" . $p->resp_sal . "', estado='S' WHERE id_entsal=" . $p->id_entsal;
  	mysql_query($sql);
  ...
  public function method_leer_vehiculo($params, $error) {
	...
	function functionAux1(&$row, $key) {
  	...
  	$resultado->vehiculo = $this->toJson($sql, $opciones);
  	$resultado->vehiculo = $resultado->vehiculo[0];
  	
  	
  	
	function functionAux2(&$row, $key) {
		$row->total = (float) $row->total;
  	...
  	$opciones = new stdClass;
  	$opciones->functionAux = functionAux2;
  	
	$sql = "SELECT * FROM entsal WHERE id_vehiculo=" . $p->id_vehiculo . " ORDER BY f_ent DESC";
	$resultado->entsal = $this->toJson($sql, $opciones);
