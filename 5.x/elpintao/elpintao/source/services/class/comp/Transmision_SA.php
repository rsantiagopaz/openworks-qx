...
class class_Transmision_SA extends class_Base
{
  ...
  public function method_transmitir_stock($params, $error) {
	...
	$sql = "DELETE FROM transmision_error WHERE tipo='stock'";
	mysql_query($sql, $this->link1);
		...
		if (mysql_num_rows($rs) > 0) {
			...
			if ($link2) {
				try {
					...
					$sql = "INSERT transmision_error SET id_sucursal=" . $id_sucursal2 . ", tipo='stock', hora='" . date("H:i:s") . "', descrip='transmisión stock', detalle='" . $e->getMessage() . "'";
					mysql_query($sql, $this->link1);
				}
			} else {
				$sql = "INSERT transmision_error SET id_sucursal=" . $id_sucursal2 . ", tipo='stock', hora='" . date("H:i:s") . "', descrip='conexión remota', detalle='" . $php_errormsg . "'";
				mysql_query($sql, $this->link1);
			}
		}
	}
