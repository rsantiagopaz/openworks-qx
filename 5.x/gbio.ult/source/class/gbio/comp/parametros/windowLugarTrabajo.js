qx.Class.define("gbio.comp.parametros.windowLugarTrabajo",
{
	...
	commandAgregar.addListener("execute", function(e){
		...
		tableModel.addRowsAsMapArray([{id_lugar_trabajo: "0", descrip: "Nuevo lugar de trabajo", alta: true}], null, true);
	...
	var btnAgregar = new qx.ui.menu.Button("Agregar lugar de trabajo", null, commandAgregar);
	...
	menu.memorizar();
		...
		aux.setValidationFunction(function(newValue, oldValue){
			newValue = newValue.trim().substr(0, 50);
		...
		btnAceptar.addListener("execute", function(e){
			...
			if (enviar) {
				...
				try {
					var resultado = rpc.callSync("escribir_lugar_trabajo", p);
				} catch (ex) {
