...
class class_Vehiculo extends class_Base
{
  ...
  public function method_leer_gral($params, $error) {
	...
	$sql = "SELECT id_entsal, CONCAT(nro_patente, '  ', marca) AS vehiculo, f_ent, f_sal, asunto, entsal.estado FROM entsal INNER JOIN vehiculo USING(id_vehiculo) WHERE vehiculo.id_parque=" . $_SESSION['parque']->id_parque . " AND (entsal.estado <> 'S' OR entsal.asunto) ORDER BY f_ent DESC";
	$rs = mysql_query($sql);
