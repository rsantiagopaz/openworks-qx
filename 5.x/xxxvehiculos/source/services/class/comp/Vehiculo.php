...
class class_Vehiculo extends class_Base
{
  ...
  public function method_leer_movimiento($params, $error) {
	...
	function functionAux1(&$row, $key) {
		...
		$row->total = (($row->estado == "D") ? "Diferido" : (float) $row->total);
	};
  ...
  public function method_entrada_vehiculo($params, $error) {
  	...
  	if ($row->estado == "S") {
	  	...
	  	$sql = "INSERT entsal SET id_vehiculo=" . $p->id_vehiculo . ", observa='" . $p->observa . "', f_ent=NOW(), id_usuario_ent='" . $_SESSION['usuario'] . "', resp_ent='" . $p->resp_ent . "', kilo=" . $p->kilo . ", asunto=FALSE, diferido=FALSE, estado='E'";
	  	mysql_query($sql);
	  	$insert_id = mysql_insert_id();
