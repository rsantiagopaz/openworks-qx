...
case "permisos" : {
	...
	<thead>
	<tr><th>descripcion</th><th>entrada 1</th><th>entrada 2</th><th>fichaje</th><th>empleado</th></tr>
	</thead>
	<tbody>
	<?php
	...
	$rsTurnos = mysql_query($sql);
	while ($rowTurnos = mysql_fetch_object($rsTurnos)) {
		$sql = "SELECT empleado.*, tolerancia.* FROM (empleado INNER JOIN empleado_turno USING(id_empleado)) INNER JOIN tolerancia USING(id_tolerancia) WHERE id_turno=" . $rowTurnos->id_turno . " ORDER BY name";
		$rsEmpleado = mysql_query($sql);
		while ($rowEmpleado = mysql_fetch_object($rsEmpleado)) {
			$rowEmpleado->e_fichada = (int) $rowEmpleado->e_fichada;
			$rowEmpleado->e_tolerable = (int) $rowEmpleado->e_tolerable;
			$rowEmpleado->e_tardanza = (int) $rowEmpleado->e_tardanza;
			$rowEmpleado->e_30minutos = (int) $rowEmpleado->e_30minutos;
			$rowEmpleado->e_60minutos = (int) $rowEmpleado->e_60minutos;
			
			$rowEmpleado->control_entrada = (bool) $rowEmpleado->control_entrada;
			$rowEmpleado->control_salida = (bool) $rowEmpleado->control_salida;
			$rowEmpleado->total_minutos = (int) $rowEmpleado->total_minutos;
			$rowEmpleado->limite_tardanzas = (int) $rowEmpleado->limite_tardanzas;
			
			
			$sql = "SELECT * FROM fichaje INNER JOIN empleado_reloj USING(id_empleado_reloj) WHERE id_empleado=" . $rowEmpleado->id_empleado . " AND inout_mode=0 ORDER BY fecha_hora";
			$rsFichaje = mysql_query($sql);
			while ($rowFichaje = mysql_fetch_object($rsFichaje)) {
				...
				$fichaje = new DateTimeEnhanced($rowFichaje->fecha_hora);
				$mes = $fichaje->format("m");
				$ano = $fichaje->format("Y");
				
				if ($mes != $ultimo_mes || $ano != $ultimo_ano) {
					$ultimo_mes = $mes;
					$ultimo_ano = $ano;
					
					$total_minutos = $rowEmpleado->total_minutos;
					$limite_tardanzas_contador = 0;
				}
				
				
				$entrada = clone $fichaje;
				$entrada->setTime((int) substr($rowTurnos->entrada, 0, 2), (int) substr($rowTurnos->entrada, 3, 2), (int) substr($rowTurnos->entrada, 6, 2));
				

				$entrada_aux1 = $entrada->returnAdd(new DateInterval("PT" . ($rowEmpleado->e_tolerable + 1) . "M"));
				$entrada_aux2 = $entrada_aux1->returnAdd(new DateInterval("PT" . $rowEmpleado->e_tardanza . "M"));
				if ($entrada_aux1 <= $fichaje && $fichaje <= $entrada_aux2) {
					$limite_tardanzas_contador = $limite_tardanzas_contador + 1;
					
					if ($limite_tardanzas_contador <= $rowEmpleado->limite_tardanzas) {
						?>
						<tr><td>Tardanza</td><td><?php echo $entrada_aux1->format("Y-m-d H:i:s"); ?></td><td><?php echo $entrada_aux2->format("Y-m-d H:i:s"); ?></td><td><?php echo $fichaje->format("Y-m-d H:i:s"); ?></td><td><?php echo $rowEmpleado->name; ?></td></tr>
						<?php
					} else {
						$total_minutos = $total_minutos - 30;
						
						?>
						<tr><td>Tardanza resta 30</td><td><?php echo $entrada_aux1->format("Y-m-d H:i:s"); ?></td><td><?php echo $entrada_aux2->format("Y-m-d H:i:s"); ?></td><td><?php echo $fichaje->format("Y-m-d H:i:s"); ?></td><td><?php echo $rowEmpleado->name; ?></td></tr>
						<?php
					}
				}
				
				
				$entrada_aux1 = $entrada_aux2->returnAdd(new DateInterval("PT1M"));
				$entrada_aux2 = $entrada_aux1->returnAdd(new DateInterval("PT" . $rowEmpleado->e_30minutos . "M"));
				if ($entrada_aux1 <= $fichaje && $fichaje <= $entrada_aux2) {
					$total_minutos = $total_minutos - 30;
					
					?>
					<tr><td>Resta 30</td><td><?php echo $entrada_aux1->format("Y-m-d H:i:s"); ?></td><td><?php echo $entrada_aux2->format("Y-m-d H:i:s"); ?></td><td><?php echo $fichaje->format("Y-m-d H:i:s"); ?></td><td><?php echo $rowEmpleado->name; ?></td></tr>
					<?php
				}
				
				
				$entrada_aux1 = $entrada_aux2->returnAdd(new DateInterval("PT1M"));
				$entrada_aux2 = $entrada_aux1->returnAdd(new DateInterval("PT" . $rowEmpleado->e_60minutos . "M"));
				if ($entrada_aux1 <= $fichaje && $fichaje <= $entrada_aux2) {
					$total_minutos = $total_minutos - 60;
					
					?>
					<tr><td>Resta 60</td><td><?php echo $entrada_aux1->format("Y-m-d H:i:s"); ?></td><td><?php echo $entrada_aux2->format("Y-m-d H:i:s"); ?></td><td><?php echo $fichaje->format("Y-m-d H:i:s"); ?></td><td><?php echo $rowEmpleado->name; ?></td></tr>
					<?php
				}
				
				
				if ($entrada_aux2 < $fichaje) {
					?>
					<tr><td>Sanción</td><td><?php echo $entrada_aux1->format("Y-m-d H:i:s"); ?></td><td><?php echo $entrada_aux2->format("Y-m-d H:i:s"); ?></td><td><?php echo $fichaje->format("Y-m-d H:i:s"); ?></td><td><?php echo $rowEmpleado->name; ?></td></tr>
					<?php
				}
			}
		}
