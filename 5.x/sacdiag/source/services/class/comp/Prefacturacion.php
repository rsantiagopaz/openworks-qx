...
class class_Prefacturacion extends class_Base
{
  ...
  public function method_leer_prefacturacion($params, $error) {
	...
	$sql = "SELECT * FROM prefacturaciones WHERE estado<>'P' ORDER BY fecha_creacion DESC";
	
	$rs = $this->mysqli->query($sql);
	while ($row = $rs->fetch_object()) {
		$sql = "SELECT _organismos_areas.organismo_area AS prestador FROM prefacturaciones_items INNER JOIN solicitudes USING(id_solicitud) INNER JOIN _organismos_areas ON solicitudes.id_prestador = _organismos_areas.organismo_area_id WHERE prefacturaciones_items.id_prefacturacion='" . $row->id_prefacturacion . "'";
		$rsAux = $this->mysqli->query($sql);
		$rowAux = $rsAux->fetch_object();
